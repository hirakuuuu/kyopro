...
int main(){
    ...
    LazySegmentTree<S, op, e, F, mapping, composition, id> lst(w);
    vector<vector<pair<int, int>>> pos(n+1);
    pos[0].push_back({0, 0});
    rep(i, 0, h){
        for(auto c: coin[i]){
            lst.apply(c, w, lst.get(c)+1);
            pos[lst.get(c)].push_back({i, c});
        }
    }
    cout << lst.get(w-1) << endl;
    
    // 復元
    string s;
    pair<int, int> tmp = {h-1, w-1};
    int cnt = lst.get(w-1)+1;
    while(cnt > 0){
        // cnt -> cnt-1 に向かう
        for(auto [nx, ny]: pos[cnt-1]){
            if(nx > tmp.first || ny > tmp.second) continue;
            while(tmp.first > nx) s += "D", tmp.first--;
            while(tmp.second > ny) s += "R", tmp.second--;
            break;
        }
        cnt--;
    }
