...
class RollingHash {
    ...
    void build(const Str &s){
        ...
        hash[0] = 0;
        for(int i = 0; i < n; i++){
            hash[i+1] = calcMod(mul(hash[i], base)+s[i]-'a');
...
int main(){
    ...
    RollingHash<string> rev_t(t, b);


    rep(i, 0, n+1){
        // unite で離れた区間を結合できる（覚えろ）
        ull h1 = rh_t.unite(rh_t.get(0, i), rh_t.get(i+n, 2*n), n-i);
        ull h2 = rev_t.get(n-i, 2*n-i);
        if(h1 == h2){
