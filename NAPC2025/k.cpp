...
int main(){
    ...
    while(t--){
        ...
        rep(i, 0, sz_x) id_x[X[i]] = i;
        rep(i, 0, sz_y) id_y[Y[i]] = i;

        Dinic<ll> g(1+sz_x+sz_y+1);
        int source = sz_x+sz_y, target = source+1;
        rep(i, 0, sz_x){
            g.add_edge(source, i, 1);
        }
        rep(i, 0, n){
            if(x[i] == -IINF) continue;
            g.add_edge(id_x[x[i]], sz_x+id_y[y[i]], 1);
        }
        rep(i, 0, sz_y){
            g.add_edge(sz_x+i, target, 1);
        }
