...
/*
遅延セグ木には {0乗和, 1乗和, 2乗和, ... , k常和} がのる
https://drken1215.hatenablog.com/entry/2024/06/09/022229
*/

// 遅延セグメント木の準備
using S = tuple<mint, mint, int>; // {a*a, a, 1} の総和
S op(S a, S b) {
    auto [a1, a2, sa] = a;
    auto [b1, b2, sb] = b;
...
S mapping(F a, S x) {
    auto [x1, x2, sx] = x;
    return {x1+x2*a*2+a*a*sx, x2+a*sx, sx};
}
// a(b(x)) という包含関係
...
// verify: https://atcoder.jp/contests/abc455/tasks/abc455_f
int main(){
    int n, q; cin >> n >> q;
    vector<mint> a(n);
    vector<S> init(n);
    rep(i, 0, n){
        init[i] = {a[i]*a[i], a[i], 1};
    }
    ...
    while(q--){
        ...
        mint ma = mint(b);
        lst.apply(l, r, ma);
        auto [AA, A, _] = lst.prod(l, r);
        // cout << AA.val() << ' ' << A.val() << endl;
        cout << ((A*A-AA)/2).val() << endl;
    }
