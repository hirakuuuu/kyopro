...
// カタラン数の三角形
mint catalan_triangle(int n, int k){
    if(k < 0 || n < k || (n-k)%2 == 1) return 0;
    return mint(k+1)*choose(n+1, (n-k)/2)/(n+1);
}
...
int main(){
    ...
    while(t--){
        int n, k; cin >> n >> k;
        mint tmp = catalan_triangle(n, 2*k-n);
        cout << tmp.pow(2).val() << endl;
    }
    return 0;
}