...
ランダムに選んだ1～10マスを変更
スコアが上がれば更新、そうでなければ減少量に応じた確率で更新する焼き鈍し
*/

const int n = 30;
const int TIME_LIMIT = 2990, INF = pow(10, 9)+7;
const double start_temp = 10, end_temp = 5;
vector<int> b(3);
...
int get_score(vector<vector<int>> a, int num, vector<int> h, vector<int> w){
    int sum_a = 0;
    vector<int> cnt(3), checked_h(n, 0), checked_w(n, 0);
    rep(i, 0, num){
        if(checked_h[h[i]] == 0){
            rep(j, 0, n){
                sum_a = 0;
...
int main(){
    ...
    while(true){
        ...
        int nscore = get_score(cp_a, num, h, w);
        int pscore = get_score(a, num, h, w);
        double new_score = nscore;
        double pre_score = pscore;
        // 温度関数
