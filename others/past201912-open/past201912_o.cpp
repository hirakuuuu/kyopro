...
int main(){
    ...
    SegmentTree<double, op, e> st(vector<double>(n, 1.0));
    vector<double> sum(n);
    rep(i, 0, 6*n){
        auto [b, id] = d[i];
        double e = st.all_prod();
        double tmp = st.get(id);
        st.set(id, tmp-1.0/6.0+(e+1.0)/6.0);
        sum[id] += e;
        cout << e << ' ' << b << ' ' << id << endl;
    }
    double ans = 0;
    rep(i, 0, n) chmax(ans, sum[i]/6.0);
    printf("%.10f\n", ans+1.0);
