...
int main(){
    int n, q; cin >> n >> q;
    vector<int> p(n);
    vector<int> acc_p(n+1);
    rep(i, 0, n){
        cin >> p[i];
        p[i]--;
        acc_p[i+1] = (acc_p[i]+p[i])%3; 
    }

    vector<vector<mint>> dp(n+1, vector<mint>(3));
    rep(i, 1, n+1){
        {
            if(p[i-1] == 0) dp[i][0] = dp[i-1][0];
            if(p[i-1] == 1) dp[i][0] = dp[i-1][2]+mint::modpow(2, i-1);
            if(p[i-1] == 2) dp[i][0] = dp[i-1][1]+mint::modpow(2, i-1);
        }
        {
            if(p[i-1] == 0) dp[i][1] = dp[i-1][2]+mint::modpow(2, i-1);
            if(p[i-1] == 1) dp[i][1] = dp[i-1][1];
            if(p[i-1] == 2) dp[i][1] = dp[i-1][0]+mint::modpow(2, i-1);
        }
        {
            if(p[i-1] == 0) dp[i][2] = dp[i-1][1]+mint::modpow(2, i-1);
            if(p[i-1] == 1) dp[i][2] = dp[i-1][0]+mint::modpow(2, i-1);
            if(p[i-1] == 2) dp[i][2] = dp[i-1][2];
        }
    ...
    while(q--){
        ...
        if(c == 'c'){
            ...
            int l, r; cin >> l >> r; l--;
            cout << (dp[r][0]-dp[l][(acc_p[l]+1)%3])/mint::modpow(2, l) << endl;
        }
    }
    return 0;
}