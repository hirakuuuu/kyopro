...
int main(){
    ...
    UnionFind uf_r(n), uf_c(n);
    rep(i, 0, n) rep(j, i+1, n){
        bool ok = true;
        rep(l, 0, n){
            if(a[i][l]+a[j][l] > k) ok = false;
        }
        if(ok) uf_r.unite(i, j);
    }
    rep(i, 0, n) rep(j, i+1, n){
        bool ok = true;
        rep(l, 0, n){
            if(a[l][i]+a[l][j] > k) ok = false;
        }
        if(ok) uf_c.unite(i, j);
    }

    mint ans = 1;
    rep(i, 0, n){
        if(uf_r[i] == i) ans *= fact[uf_r.size(i)];
        if(uf_c[i] == i) ans *= fact[uf_c.size(i)];
    }
    cout << ans.val() << endl;
    return 0;
}