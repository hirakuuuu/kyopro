...
int main(){
    ...
    // 全部同じにできるかどうか
    ll tot = 0;
    rep(i, 0, n) tot += mx-a[i];
    if(tot <= k){
        cout << mx+(k-tot)/n << endl;
        return 0;
    }
    ...
    rep(i, 0, n) lst.set(i, {mx-a[i], 1});
    rrep(i, mx-1, 1){
        {
            int l = 0, r = 0;
            for(ll j = i+1; l < n; j += i+1){
                r = upper_bound(a.begin()+l, a.end(), j)-a.begin();
                lst.apply(l, r, -j);
                l = r;
            }
        }
