...
using F = int;
S mapping(F a, S x) { return max(0, x-a); }
// a(b(x)) という包含関係
...
int main(){
    ...
    LazySegmentTree<S, op, e, F, mapping, composition, id> lst(2*n);
    rep(i, 0, n){
        lst.set(i, a[i]);
        lst.set(i+n, a[i]);
    }
    ...
    rep(i, 0, m){
        rep(j, 0, c[i].size()){
            if(lst.get(c[i][j]) == 0) continue;
            // lst.set(c[i][j], 0);
            // lst.set(c[i][j]+n, 0);
            // mid = c[i][j] として、操作可能な区間を1減らす
            int mid = c[i][j]+n;
            while(0 <= mid && lst.get(mid) == 1) mid--;
            int l = lst.min_left<f>(mid+1);
            mid = c[i][j];
            while(mid < 2*n && lst.get(mid) == 1) mid++;
            int r = lst.max_right<f>(mid);
            cout << l << ' ' << r << endl;
            if(r <= l){
                if(n <= r){
                    lst.apply(0, r-n, 1);
