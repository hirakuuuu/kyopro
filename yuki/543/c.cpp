...
int main(){
    ...
    rep(i, 0, n) st.set(a[i], {i, a[i]});

    vector<vector<int>> g(n);
    auto f = [&](auto self, int l, int r, int par) -> void {
        // cout << l << ' ' << r << ' ' << par << endl;
        {
            auto [val, id] = st.prod(l, par);
            if(val != IINF){
                g[par].push_back(id);
                self(self, l, par, id);
            }
        ...
        {
            auto [val, id] = st.prod(par+1, r);
            if(val != IINF){
                g[par].push_back(id);
                self(self, par+1, r, id);
            }
        }
    };
    ...
    rep(i, 0, n) cout << b[a[i]] << ' ';
    cout << endl;
    rep(i, 0, n) cout << c[a[i]] << ' ';
    cout << endl;
