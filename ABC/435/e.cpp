...
int main(){
    ...
    map<int, int> pos;
    int ind = 0;
    for(auto pp: p) pos[pp] = ind++;

    LazySegmentTree<S, op, e, F, mapping, composition, id> lst(p.size());
    rep(i, 0, (int)p.size()-1){
        lst.set(i, {p[i+1]-p[i], 1});
    }

    rep(i, 0, q){
        lst.apply(pos[l[i]-1], pos[r[i]], 0);
        cout << lst.all_prod().first << endl;
    }
