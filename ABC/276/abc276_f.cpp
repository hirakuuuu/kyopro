...
int main(){
    ...
    vector<ll> a(n);
    rep(i, 0, n) cin >> a[i];
    SegmentTree<pair<mint, ll>, op, e> st(200005);
    mint e = 0;
    rep(i, 0, n){
        pair<mint, ll> tmp = st.prod(a[i], 200005);
        e += tmp.first*2;
        mint cur = a[i]*(2*(i-tmp.second)+1);
        e += cur;
        cout << e/((i+1)*(i+1)) << endl;
        
        pair<mint, ll> pre = st.get(a[i]);
        st.set(a[i], {pre.first+a[i], pre.second+1LL});
    }
