...
S op(S a, S b) {
    ...
    ll al = a.first, ar = a.first+a.second;
    ll bl = b.first, br = b.first+b.second;
    if(al <= bl && br <= ar) return a;
    return b;
}
...
F id() { return {2, 0}; }
S mapping(F a, S x) {
    if(a.first == 2) return x;
    if(a.first == 0){
        // 左端を a.second にする
        return {a.second, x.second};
    }
    // 右端を a.second にする
    return {a.second-x.second, x.second};
}
// a(b(x)) という包含関係
...
int main(){
    ...
    while(q--){
        ...
        if(t == 1){
            int v; cin >> v;
            ll l = lst.get(v-1).first;
            lst.apply(0, v, {0, l});
        }else if(t == 2){
            int v; cin >> v;
            ll r = lst.get(v-1).first+lst.get(v-1).second;
            lst.apply(0, v, {1, r});
        }else{
            ll x; cin >> x;
            int r = lst.max_right(0, [&](pair<ll, ll> tmp){return !inr(tmp.first, x, tmp.first+tmp.second);});
            cout << n-r << endl;
        }
    }
    return 0;
