...
int main(){
    ...
    SegmentTree<S, op, e> st_u(n);
    SegmentTree<S, op, e> st_d(n);
    SegmentTree<S, op, e> cnt(n);
    mint ans = 0;
    rep(i, 0, n){
        st_u.add(p[i], cnt.prod(0, p[i]));
        cnt.add(p[i], 1);
        st_d.add(p[i], st_u.prod(p[i]+1, n)+st_d.prod(p[i]+1, n));
        st_u.add(p[i], st_u.prod(0, p[i])+st_d.prod(0, p[i]));
        ans += st_d.get(p[i]);
    }
    cout << ans.val() << endl;
