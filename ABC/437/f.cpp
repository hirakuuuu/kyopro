...
int main(){
    ...
    LazySegmentTree<S, op, e, F, mapping, composition, id> xp(n);
    LazySegmentTree<S, op, e, F, mapping, composition, id> xm(n);
    LazySegmentTree<S, op, e, F, mapping, composition, id> yp(n);
    LazySegmentTree<S, op, e, F, mapping, composition, id> ym(n);
    rep(i, 0, n){
        xp.set(i, x[i]-y[i]);
        xm.set(i, -x[i]+y[i]);
        yp.set(i, x[i]+y[i]);
    ...
    while(q--){
        int t; cin >> t;
        if(t == 1){
            int i, nx, ny; cin >> i >> nx >> ny;
            i--;
            xp.apply(i, i+1, -x[i]+y[i]);
            xm.apply(i, i+1, x[i]-y[i]);
            yp.apply(i, i+1, -x[i]-y[i]);
            ym.apply(i, i+1, x[i]+y[i]);
            x[i] = nx, y[i] = ny;
            xp.apply(i, i+1, x[i]-y[i]);
            xm.apply(i, i+1, -x[i]+y[i]);
            yp.apply(i, i+1, x[i]+y[i]);
            ...
            ll ans = 0;
            chmax(ans, xp.prod(L, R));
            chmax(ans, xm.prod(L, R));
            chmax(ans, yp.prod(L, R));
            chmax(ans, ym.prod(L, R));
            cout << ans << endl;
