...
int main(){
    ...
    while(q--){
        int s, t; cin >> s >> t; s--, t--;
        if(cnt[{s, t}] >= 2 || (cnt[{s, t}] == 1 && (seg.prod(s, t+1) < t || seg.prod(s+1, t+1) <= t))){
            cout << "Yes" << endl;
            continue;
        }

        int j = lower_bound(r[s].begin(), r[s].end(), make_pair(-t, -1))-r[s].begin();
        int k = lower_bound(l[t].begin(), l[t].end(), make_pair(s, -1))-l[t].begin();
        if(j >= r[s].size() || k >= l[t].size()){
            cout << "No" << endl;
            continue;
        } 
        if(l[t][k].first > -r[s][j].first+1){
            cout << "No" << endl;
        }else{
            cout << "Yes" << endl;
        }
