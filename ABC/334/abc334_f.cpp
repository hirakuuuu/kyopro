...
vector<double> x(250005), y(250005);
...
int main(){
    ...
    vector<double> santa(n+1);
    rep(i, 1, n+1){
        if(i != n) santa[i] = dist(0, i)+dist(0, i+1)-dist(i, i+1);
    }
    vector<double> dp(k);
    double tmp = dist(0, 1);
    rep(i, 1, k+1) tmp += dist(i, (i+1)%(n+1));
    rep(i, 1, k+1){
        dp[i%k] = tmp+santa[i];
    }
    LazySegmentTree<double, op, e, double, mapping, composition, id> st(dp);

    int offset = 0;
    rep(i, k+1, n+1){
        offset++;
        st.set(offset%k, st.all_prod()+santa[i]);
        st.apply(0, k, dist(i, (i+1)%(n+1)));
    }
