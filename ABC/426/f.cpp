...
int main(){
    ...
    vector<vector<pair<int, ll>>> ql(n+1);
    vector<vector<pair<int, ll>>> qr(n+1);
    rep(i, 0, q){
        int l, r; cin >> l >> r; l--;
        ll k; cin >> k;
        query[i] = {l, r, k};
    ...
    rep(i, 0, n){
        ...
        int r = st.max_right(0, [&](S tmp){return tmp <= a[i];});
        // cout << i << ' ' << r << ' ' << a[i]-st.prod(0, r) << endl;
        ans[r] += a[i]-st.prod(0, r);
        g[r].push_back(i);
    }

    SegmentTree<S, op, e> st2(vector<S>(n, 1));
    rep(i, 0, q){
        for(auto id: g[i]){
            st2.add(id, -1);
        }
        auto [l, r, k] = query[i];
        ans[i] += st2.prod(l, r)*k;
        cout << ans[i] << endl;
    }
