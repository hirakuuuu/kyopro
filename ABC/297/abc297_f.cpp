...
int main(){
    ...
    vector<vector<mint>> sum(h+1, vector<mint>(w+1));
    mint ans = 0;
    initfact();
    rep(i, 1, h+1){
        rep(j, 1, w+1){
            mint cnt = choose(i*j, k);
            if(i >= 2 && j >= 2) cnt += -sum[i-1][j]*2+sum[i-2][j]-sum[i][j-1]*2+sum[i][j-2]-sum[i-2][j-2];
            if(i >= 2) cnt += -sum[i-1][j]*2+sum[i-2][j];
            if(j >= 2) cnt += -sum[i][j-1]*2+sum[i][j-2];
            mint area = i*j;
            area *= (h-i+1)*(w-j+1);
            ans += cnt*area;
            cout << i << ' ' << j << ' ' << cnt << endl;
