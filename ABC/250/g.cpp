...
// 遅延セグメント木の準備
using S =  pair<ll, ll>;
S op(S a, S b) { return {max(a.first, b.first), min(a.second, b.second)}; }
S e() { return {-1e18, 1e18}; }
// 一次関数 a x + b によって恒等写像と代入を表現
using F = ll;
S mapping(F a, S x) { return {x.first+a, x.second+a}; }
// a(b(x)) という包含関係
...
int main(){
    ...
    vector<S> s(n+1);
    vector<int> h(n, 1);
    rep(i, 0, n) s[i] = {i+1, i+1};
    LazySegmentTree<S, op, e, F, mapping, composition, id> lst(s);
    rep(i, 0, n){
        auto [p, d] = pd[i];
        d *= -1;
        lst.apply(d, n, -2);
        S tmp = lst.all_prod();
        // rep(j, 0, n){
        //     cout << '(' << lst.get(j).first << ' ' << lst.get(j).second << "), "; 
        // }
        // cout << tmp.first << ' ' << tmp.second << endl;
        if(i+1 > n-i-1 || tmp.first-tmp.second >= n-1-i){
            if(i+1 <= n-i-1) h[d] = 0;
            lst.apply(d, n, 1);
        }else{
            h[d] = -1;
