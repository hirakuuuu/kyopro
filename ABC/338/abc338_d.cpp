...
int main(){
    ...
    vector<ll> init(n);
    LazySegmentTree<ll, op, e, ll, mapping, composition, id> lst(init);

    rep(i, 0, m-1){
        if(x[i] < x[i+1]){
            lst.apply(0, x[i], x[i+1]-x[i]);
            lst.apply(x[i], x[i+1], n-(x[i+1]-x[i]));
            lst.apply(x[i+1], n, x[i+1]-x[i]);
        }else{
            lst.apply(0, x[i+1], x[i]-x[i+1]);
            lst.apply(x[i+1], x[i], n-(x[i]-x[i+1]));
            lst.apply(x[i], n, x[i]-x[i+1]);
        }
    }
