...
int main(){
    ...
    LazySegmentTree<S, op, e, F, mapping, composition, id> seg(1000001);
    if(st[0].first <= st[0].second-d+1){
        seg.apply(st[0].first, st[0].second-d+1, 1);
    }
    ll ans = 0;
    rep(i, 1, n){
        if(st[i].first <= st[i].second-d+1){
            ans += seg.prod(st[i].first, st[i].second-d+1).first;
            seg.apply(st[i].first, st[i].second-d+1, 1);
        }
    }
