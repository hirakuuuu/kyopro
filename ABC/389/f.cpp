...
int main(){
    ...
    LazySegmentTree<S, op, e, F, mapping, composition, id> lst(vec);
    rep(i, 0, n){
        int L = lst.max_right(0, [&](int tmp){return tmp < l[i];});
        int R = lst.max_right(0, [&](int tmp){return tmp < r[i]+1;});
        lst.apply(L, R, 1);
    }
    

    rep(i, 0, q){
        cout << lst.get(x[i]) << endl;
    }
    return 0;
}