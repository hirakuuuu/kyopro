...
int main(){
    int n, q; cin >> n >> q;
    SegmentTree<S, op, e> seg_r(q), seg_c(q);

    ll ans = 0;
    vector<int> pre_r(n, -1);
    ...
    rep(i, 0, q){
        ...
        if(t == 1){
            int r; cin >> r; r--;
            if(pre_r[r] != -1){
                int tmp = seg_c.prod(pre_r[r], i);
                ans += tmp;
            }else{
                ans += n;
            }
            if(pre_r[r] != -1) seg_r.set(pre_r[r], 0);
            pre_r[r] = i;
            seg_r.set(pre_r[r], 1);
        }else{
            int c; cin >> c; c--;
            if(pre_c[c] != -1){
                int tmp = seg_r.prod(pre_c[c], i);
                ans -= tmp;
            }
            if(pre_c[c] != -1) seg_c.set(pre_c[c], 0);
            pre_c[c] = i;
            seg_c.set(pre_c[c], 1);
        }
        cout << ans << endl;
