...
// 遅延セグメント木の準備
S op_mx(S a, S b) { return max(a, b); }
S e_mx() { return -9e18; }
...
int main(){
    ...
    LazySegmentTree<S, op_mx, e_mx, F, mapping, composition, id> lst(n+1);
    ll acc = 0;
    rep(i, 0, n+1){
        lst.set(i, acc+a[i]);
        acc += a[i];
    ...
    while(q--){
        ...
        ll x; cin >> x;
        x -= b;
        lst.apply(c, n+1, -a[c]+x);
        st.set(c, x);
        a[c] = x;
        int ng = 0, ok = n+1;
        while(ok-ng > 1){
            int mid = (ok+ng)/2;
            if(lst.prod(0, mid) >= 0) ok = mid;
            else ng = mid;
        }
        if(ok == n+1) ok = n;
        long double ans = ((long double)st.prod(0, ok)+(long double)b*ok)/ok;
