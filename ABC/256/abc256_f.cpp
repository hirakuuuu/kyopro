...
int main(){
    ...
    SegmentTree<mint, op, e> st_a(a), st_ia(ia), st_iia(iia);
    ...
    while(q--){
        ...
        if(t == 1){
            ...
            st_a.set(x, y);
            st_ia.set(x, mx*y);
            st_iia.set(x, mx*mx*y);
        }else if(t == 2){
            int x; cin >> x;
            mint mx = x;
            mint ans = st_iia.prod(0, x);
            ans -= (mx*2+3)*st_ia.prod(0, x);
            ans += (mx*mx+mx*3+2)*st_a.prod(0, x);
            ans /= 2;
            cout << ans << endl;
        }
    }
