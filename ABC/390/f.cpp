...
int main(){
    ...
    auto update = [&](int i) -> void {
        if(i == n) return;
        int f = 0;
        if(a[i] > 0 && pos[a[i]-1][p[a[i]-1]] < i) f++;
        if(a[i] < n-1 && pos[a[i]+1][p[a[i]+1]] < i) f++;
        st.set(i, (ll)(1-f)*(n-i));
        return;
    };
    ...
    rep(i, 0, n){
        ...
        p[a[i]]++;
        update(pos[a[i]][p[a[i]]]);
        if(a[i] > 0) update(pos[a[i]-1][p[a[i]-1]]);
        if(a[i] < n-1) update(pos[a[i]+1][p[a[i]+1]]);
    }
    cout << ans << endl;
    return 0;
}