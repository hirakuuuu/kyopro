...
// reference: https://lorent-kyopro.hatenablog.com/entry/2021/03/12/025644
...
int main(){
    ...
    dynamic_segtree<S, op, e> st(1000000000+1);
    ll ans = n*(n+1)/2;
    ll mx = -1;
    rep(i, 0, n){
        ll l = st.prod(max(a[i]-d+1, 0LL), min(a[i]+d, (ll)1000000000+1));
        chmax(mx, l);
        ans -= mx+1;
        st.set(a[i], i);
    }
