...
// 遅延セグメント木の準備
ll op(ll a, ll) { return a; }
ll e() { return {}; }
// 一次関数 a x + b によって恒等写像と代入を表現
ll mapping(pll a, ll x) { return a.first * x + a.second; }
pll composition(pll a, pll b) { return {a.first * b.first, a.first * b.second + a.second}; }
pll id() { return {1, 0}; }
...
int main(){
    ...
    LazySegmentTree<ll, op, e, pll, mapping, composition, id> lazyseg(a);
    ...
    while(q--){
        ...
        if(t == 1){
            int x; cin >> x;
            lazyseg.apply(0, n+1, {0, x});
        }else if(t == 2){
            int idx, x; cin >> idx >> x;
            lazyseg.set(idx, lazyseg.get(idx)+x);
        }else{
            int idx; cin >> idx;
            cout << lazyseg.get(idx) << endl;
        }
    }
