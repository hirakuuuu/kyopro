...
int main(){
    ...
    rep(i, 0, n){
        // セグ木上の二分探索
        l[i] = st.min_left(i+1, [&](auto tmp){return tmp <= a[i];});
        r[i] = st.max_right(i, [&](auto tmp){return tmp <= a[i];});
    }

    vector<ll> acc(n+3);
    rep(i, 0, n){
        int l1 = min(i-l[i], r[i]-i-1), l2 = max(i-l[i], r[i]-i-1);
        // cout << l1 << ' ' << l2 << endl;
        acc[1] += a[i].first;
        acc[l1+2] -= a[i].first;
        acc[l2+2] -= a[i].first;
        acc[l1+l2+3] += a[i].first;

        // vector<ll> _acc = acc;
        // rep(j, 1, n+1) _acc[j+1] += _acc[j];
        // rep(j, 1, n+1) _acc[j+1] += _acc[j];
        // rep(j, 1, n+1){
        //     cout << _acc[j] << ' ';
        // }
