...
int main(){
    ...
    vector<int> depth(n);
    vector<pair<ll, ll>> init(n, {0LL, 1LL});
    depth[0] = 0;
    ...
    rep(i, 0, n){
        lst.apply(hldmap[i], init[i].first);
    }
    ...
    while(q--){
        ...
        if(t == 1){
            int x; cin >> x; x--;
            int w; cin >> w;
            vector<pair<int, int>> p = hld.query(0, x);
            for(auto [l, r]: p){
                // cout << l << ' ' << r+1 << endl;
                lst.apply(l, r+1, w);
            }
            ...
            int y; cin >> y; y--;
            // cout << edges[y].second << endl;
            // vector<pair<int, int>> p = g.get_v(edges[y].first, edges[y].second);
            // cout << p[0].first << ' ' << p[0].second << endl;
            // cout << lst.prod(p[0].first, p[0].second).first << endl;
            cout << abs(tot-2*lst.get(hldmap[edges[y].second]).first) << endl;
        }
        // rep(i, 0, n){
        //     cout << lst.get(hldmap[i]).first << ' ';
        // }
        // cout << endl;
    }
    return 0;
