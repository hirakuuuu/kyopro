...
S op(S a, S b){
    S c(2, vector<ll>(2, INF));
    rep(i, 0, 2){
        rep(j, 0, 2){
            rep(k, 0, 2){
                chmin(c[i][j], b[i][k]+a[k][j]);
            }
        }
    }
    return c;
...
int main(){
    ...
    while(t--){
        ...
        // セグ木は構築が O(n) なので，モノイドが重い時は最初に vector を作ってから渡した方が早い
        vector<S> vec(n, e());
        rep(i, 0, n){
            vec[i][0][0] = INF;
            vec[i][0][1] = 0;
        ...
        SegmentTree<S, op, e> seg(vec);

        ll ans = INF;
        rep(l, 0, n){
            int r = l+k;
            if(r > n) break;
            vector<vector<ll>> res = seg.prod(l, r);
            chmin(ans, res[1][0]);
            if(l > 0) chmin(ans, a[l-1]+res[1][1]);
