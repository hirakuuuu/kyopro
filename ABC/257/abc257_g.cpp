...
int main(){
    ...
    vector<int> ar;
    Z_algorithm(st, ar);
    LazySegmentTree<ll, op, e, ll, mapping, composition, id> lst(t.size()+1);
    lst.set(0, 0);
    rep(i, 1, t.size()+1){
        int len = min(ar[s.size()+i-1], (int)s.size());
        ll tmp = lst.get(i-1)+1LL;
        lst.apply(i, i+len, tmp);
    }
    if(lst.get(t.size()) == id()) cout << -1 << endl;
    else cout << lst.get(t.size()) << endl;
