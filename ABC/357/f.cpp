...
// 遅延セグメント木の準備
using S = tuple<mint, mint, mint, int>; // {a*b, a, b} の総和
S op(S a, S b) {
    auto [a1, a2, a3, sa] = a;
    auto [b1, b2, b3, sb] = b;
    return {a1+b1, a2+b2, a3+b3, sa+sb};
}
S e() { return {0, 0, 0, 0}; }
...
S mapping(F a, S x) {
    auto [a1, b1] = a;
    auto [x1, x2, x3, sx] = x;
    return {x1+x3*a1+x2*b1+a1*b1*sx, x2+a1*sx, x3+b1*sx, sx};
}
// a(b(x)) という包含関係
...
int main(){
    ...
    while(q--){
        ...
        if(t == 1){
            ...
            int l, r; cin >> l >> r;
            auto [ans, _, __, ___] = lst.prod(l-1, r);
            cout << ans.val() << endl;
        }
    }
