...
int main(){
    ...
    LazySegmentTree<S, op, e, F, mapping, composition, id> lst(n+1);
    rrep(i, n-1, 0){
        rs.insert(a[i]);
        lst.set(i, (ll)rs.size());
    }
    rep(i, 0, n){
        lst.apply(i, i+1, (ll)ls.size());
        ls.insert(a[i]);
    }
    ls.clear();
    ...
    rep(i, 0, n-1){
        ls.insert(a[i]);
        lst.apply(pos[a[i]][now[a[i]]], pos[a[i]][now[a[i]]+1]+1, -1);
        now[a[i]]++;
        chmax(ans, (ll)ls.size()+lst.prod(i+2, n));
        // rep(j, 0, n){
        //     if(j < i+2) cout << "*" << ' ';
        //     else cout << lst.get(j) << ' ';
        // }
